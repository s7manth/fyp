...
if "options_disabled_2" not in st.session_state:
    st.session_state.options_disabled_2 = False

def select_option(option: str | None):
    if option is None:
        st.error("No option selected!")

    st.session_state.options_disabled_2 = True
...
with st.form("Select what you think is correct!"):
    st.radio(
        "Options",
        key="selected_option_2",
        index=0 if not st.session_state.selected_option_2 else options.index(st.session_state.selected_option_2),
        options=options,
        disabled=st.session_state.options_disabled_2
    )

    submitted = st.form_submit_button(
        "Submit",
        on_click=lambda: select_option(st.session_state.selected_option_2)
    )

if st.session_state.options_disabled_2:
    st.write(f"Selected: {st.session_state.selected_option_2}")
